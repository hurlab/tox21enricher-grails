...
#	mkdir ($outputDetailDir) || print "";
	...
	{	my @tmp1 = split (/\//, $infile);
		...
		if ($tmp3[0] =~ /__/)
		{	my @tmp4 = split (/\__/, $tmp3[0]);
			my $tmpExpFile	 = $dirInputExpression.$tmp4[0].'__ExpressionData.txt';
			if (-e $tmpExpFile)
		...
		{	my $tmpExpFile	 = $dirInputExpression.$tmp3[0].'__ExpressionData.txt';
			if (-e $tmpExpFile)
			{	$expressFile = $tmpExpFile;
			}else
		...
		{	$expressionDataExist = 1;
			open (EXP, $expressFile);
			$expressionHeader = <EXP>;
			#print $expressionHeader."\n";
			$expressionHeader =~ s/\r|\n//g;
	
			while(<EXP>)
			{	my $line = $_;
				$line =~ s/\r|\n//g;
				my @tmpSplit = split (/\t/, $line);
				if ((not defined $tmpSplit[0]) || ($tmpSplit[0] eq ""))
				{	next;
				}
		...
		{	#print "!    No expression data file for $tmp2[0] exists...\n";
		}
		...
		{	my $line = $_;
			...
			if (defined $ID2Term{$tmpSplit[1]})
			{	$pvalueMatrix{$tmpSplit[1]}->{$tmp3[0]} = -1*log10($tmpSplit[$valueColumnIndex]);
...
#	mkdir ($outputDetailDir) || print "";
	...
	open (SUMMARY, ">".$outputDir.$summaryFileName);
	@fileHeaderNames = sort_by_file_number (\@fileHeaderNames);
	print SUMMARY "CLASS\tID\tTerms\t".join("\t",@fileHeaderNames)."\n";
	...
	open (INFILE, $outputDir.$summaryFileNameBase.'__ValueMatrix.txt');
	open (OUTFILE, ">".$outputDir.$summaryFileNameBase.'__ValueMatrix.gct');
	open (DUPLICATE, ">".$outputDir.$summaryFileNameBase.'__ValueMatrix.txtduplicate');
