...
#	v2.3.1	: (09/30/2020) Changed to rely on PostgreSQL database instead of files (P.Combs)
#
#-------------------------------------------------------------------------------
...
write.table (data.frame(p.value, fdr=p.adjust(p.value)), file=\"$tempROutputFileFull\", sep=\"\\t\", row.names=FALSE, col.names=FALSE, quote=FALSE)
";
	close RCMD;
	...
	for (my $i=0; $i <= $#annoArray; $i++) {
		...
		push @{$annoArray[$i]}, $ROutputData[$i][1], $ROutputData[$i][0];
